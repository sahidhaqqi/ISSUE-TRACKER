...
def get_all_data():
    ...
    c = conn.cursor()
    c.execute(
        "SELECT id, lokasi, issue, link, progress, supeng, root_cause, tanggal FROM daily_issues ORDER BY id DESC"
    )
    rows = c.fetchall()
    conn.close()
...
def insert_data(lokasi, issue, link, progress, supeng, root_cause, tanggal):
    ...
    c = conn.cursor()
    c.execute(
        "INSERT INTO daily_issues (lokasi, issue, link, progress, supeng, root_cause, tanggal) VALUES (?, ?, ?, ?, ?, ?, ?)",
...
def update_data(id_db, lokasi, issue, link, progress, supeng, root_cause):
    ...
    c = conn.cursor()
    c.execute(
        "UPDATE daily_issues SET lokasi=?, issue=?, link=?, progress=?, supeng=?, root_cause=? WHERE id=?",
