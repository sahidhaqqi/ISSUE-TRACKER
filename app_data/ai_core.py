...
class AIProcessor:
    ...
    def run_pipeline(self, gambar_obj, mode_llm):
        ...
        if not self.ocr_model:
            raise Exception("Model OCR belum siap.")
        img_array = np.array(gambar_obj.convert('RGB'))
        result = self.ocr_model([img_array])
        raw_text_lines = [" ".join([w.value for w in line.words]) for page in result.pages for block in page.blocks for line in block.lines]
        ...
        if mode_llm == "Online (DeepSeek)":
            response = self.deepseek_client.chat.completions.create(
                model="deepseek-chat",
                messages=[{"role": "system", "content": PROMPT_SYSTEM}, {"role": "user", "content": f"Extract:\n{raw_text_trimmed}"}],
                max_tokens=120, temperature=0.1
            )
            output_llm = response.choices[0].message.content.strip()
        else:
            if not self.llm:
                raise Exception("Model Offline belum siap.")
        ...
        if not lokasi:
            ...
            if match_lokasi: lokasi = match_lokasi.group(1).strip()[:50]
